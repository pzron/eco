...
import {
  ...
  Phone, Home, Briefcase, Gift, BarChart3, DollarSign, Calendar, Percent
} from "lucide-react";
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-gradient-to-br from-primary/20 to-secondary/20 border border-white/10 rounded-2xl p-6 text-center backdrop-blur-md">
              <motion.div
                initial={{ scale: 0.9 }}
                animate={{ scale: 1 }}
                className="w-24 h-24 rounded-full bg-gradient-to-br from-primary to-secondary p-1 mx-auto mb-4"
              >
                <img
              ...
              <div className="space-y-2 mb-4">
                <Badge className="bg-gradient-to-r from-primary to-secondary border-0 mx-1">⭐ Platinum Member</Badge>
                <Badge className="bg-green-500/20 text-green-400 border border-green-500/30 mx-1">Verified</Badge>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">2,450/5,000 XP to Diamond</p>
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-md space-y-3">
              ...
              {[
                ...
                { icon: DollarSign, label: "Total Spent", value: "$15,240" },
                  ...
                  {editMode ? (
                    <div className="space-y-4">
                      <div>
                        ...
                        <input type="text" defaultValue={user?.name} className="w-full bg-black/40 border border-white/20 rounded-lg px-4 py-2 text-white placeholder:text-white/40 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30" />
                      </div>
                      <div>
                        <label className="text-sm font-semibold text-white mb-2 block">Email</label>
                        <input type="email" defaultValue={user?.email} className="w-full bg-black/40 border border-white/20 rounded-lg px-4 py-2 text-white placeholder:text-white/40 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30" />
                      </div>
                      <div>
                        <label className="text-sm font-semibold text-white mb-2 block">Phone</label>
                        <input type="tel" defaultValue="+1 (555) 000-0000" className="w-full bg-black/40 border border-white/20 rounded-lg px-4 py-2 text-white placeholder:text-white/40 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30" />
                      </div>
                      <div className="flex gap-2">
                        <Button onClick={() => setEditMode(false)} className="flex-1 bg-primary hover:bg-primary/90">Save</Button>
                  ...
                  <div className="border-t border-white/10 pt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-gradient-to-br from-purple-500/20 to-purple-500/5 border border-purple-500/30 rounded-lg p-4">
                        <p className="text-sm text-purple-200 mb-2">Total Points</p>
                      ...
                      <div className="bg-gradient-to-br from-blue-500/20 to-blue-500/5 border border-blue-500/30 rounded-lg p-4">
                        <p className="text-sm text-blue-200 mb-2">Tier Status</p>
                        <p className="text-lg font-bold text-white">Platinum ⭐</p>
                        <p className="text-xs text-blue-300">2,550 XP to Diamond</p>
                      </div>
                      <div className="bg-gradient-to-br from-green-500/20 to-green-500/5 border border-green-500/30 rounded-lg p-4">
                        <p className="text-sm text-green-200 mb-2">Referral Link</p>
                        <div className="flex items-center gap-2">
                          <code className="text-xs bg-black/40 text-primary px-2 py-1 rounded flex-1">nex-ref-{user?.id?.substring(0, 8)}</code>
                          <motion.button whileHover={{ scale: 1.1 }} onClick={() => setCopyRef(true)} className="p-1">
                ...
                <div className="p-6 space-y-4">
                  ...
                  {orders.map((order, i) => (
                    <motion.div
                      ...
                      className="bg-black/40 border border-white/10 rounded-lg p-5 hover:border-white/20 transition-all"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {wishlistItems.map((item, i) => (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.05 * i }}
                        className="bg-black/40 border border-white/10 rounded-lg p-4 hover:border-white/20 transition-all text-center"
                      >
                        ...
                        <div className="flex gap-2">
                          ...
                          <motion.button whileHover={{ scale: 1.1 }} className="px-3 h-9 hover:bg-white/10 rounded-lg transition-all">
              ...
              {activeNav === "affiliate" && (
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-gradient-to-br from-blue-500/20 to-blue-500/5 border border-blue-500/30 rounded-lg p-4">
                      <p className="text-sm text-blue-200 mb-2">Total Referrals</p>
                      <p className="text-3xl font-bold text-white">24</p>
                    </div>
                    <div className="bg-gradient-to-br from-green-500/20 to-green-500/5 border border-green-500/30 rounded-lg p-4">
                      <p className="text-sm text-green-200 mb-2">Earnings</p>
                      <p className="text-3xl font-bold text-white">$1,240</p>
                    </div>
                    <div className="bg-gradient-to-br from-purple-500/20 to-purple-500/5 border border-purple-500/30 rounded-lg p-4">
                      <p className="text-sm text-purple-200 mb-2">Commission Rate</p>
